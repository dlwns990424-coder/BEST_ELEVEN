...
import { deleteTeam, getTeamsByUserId } from "../../lib/teamStorage";
...
export default function MyTeam() {
  ...
  const handleToggleTeam = (teamId) => {
    setSelectedTeamIds((prevIds) => {
      const isSelected = prevIds.includes(teamId);
  ...
  const handleSelectAll = () => {
    ...
    const isAllSelected =
      allTeamIds.length > 0 &&
      allTeamIds.every((teamId) => selectedTeamIds.includes(teamId));
  ...
  const handleSelectedDelete = () => {
    ...
    selectedTeamIds.forEach((teamId) => {
      deleteTeam(teamId, currentUser.id);
    });
  ...
  const isAllSelected =
    ...
    sortedTeams.every((team) => selectedTeamIds.includes(team.id));
  ...
  return (
    <main className="min-h-dvh w-full bg-[#333333] text-white">
        ...
        rightAction={
          !isEditMode && teams.length > 0 ? (
            <button
              ...
              className="text-[14px] font-normal text-[#B9E000]"
            >
              편집
            </button>
          ) : null
      ...
      <section className="w-full px-5 pb-8">
        ...
        ========================= */}
        {teams.length > 0 && isEditMode && (
          <div className="mt-7 flex h-9 w-full items-center justify-end gap-5">
            <button
              type="button"
